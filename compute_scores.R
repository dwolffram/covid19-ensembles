...
df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_cum_death/df_ensembles_1wk_cum_death.csv", 
                               add_baseline = TRUE)
df_qna3 <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_cum_death/df_ensembles_1wk_cum_death_QNA3.csv", 
                          add_baseline = FALSE)
df_ensembles <- bind_rows(df_ensembles, df_qna3)
...
df_ensembles <- df_ensembles %>%
  filter(window_size == 4)
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/1wk_cum_death/ensemble_scores_1wk_cum_death.csv", row.names=FALSE)

df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_cum_death/df_ensembles_1wk_cum_death_v3-iter_refit.csv", 
                               add_baseline = FALSE)
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/1wk_cum_death/ensemble_scores_1wk_cum_death_v3-iter_refit.csv", row.names=FALSE)

df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_cum_death/df_ensembles_1wk_cum_death_top3.csv", 
                               add_baseline = FALSE)
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/1wk_cum_death/ensemble_scores_1wk_cum_death_top3.csv", row.names=FALSE)
...
df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_inc_case/df_ensembles_1wk_inc_case.csv", 
                               add_baseline = TRUE)
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/1wk_inc_case/ensemble_scores_1wk_inc_case.csv", row.names=FALSE)
...
df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_inc_death/df_ensembles_1wk_inc_death_v3-iter_refit.csv", 
                               add_baseline = FALSE)
ensemble_scores <- score_forecasts(df_ensembles)
...
df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/1wk_inc_death/df_ensembles_1wk_inc_death_v3-iter.csv", 
                               add_baseline = FALSE)

df_ensembles <- read_csv("data/ensemble_forecasts/evaluation_study/1wk_inc_death/df_ensembles_1wk_inc_death_v3-iter.csv") %>%
  select(-truth) %>% as.data.frame()
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/1wk_inc_death/ensemble_scores_1wk_inc_death_v3-iter.csv", row.names=FALSE)
...
df1 <- load_scores("scores/evaluation_study/1wk_inc_case/ensemble_scores_1wk_inc_case.csv",
                   remove_revisions=FALSE, long_format=TRUE)
df2 <- load_scores("scores/evaluation_study/1wk_inc_case/covidhub-ensemble_scores_1wk_inc_case.csv",
                   remove_revisions=FALSE, long_format=TRUE)
...
df_all <- load_scores("scores/evaluation_study/1wk_inc_death/ensemble_scores_1wk_inc_death_v3-iter.csv",
                      remove_revisions=FALSE, long_format=TRUE)
...
ggsave('plots/evaluation_study/1wk_inc_case/wis_1wk_inc_case_ws4.png', width=30, height=15, dpi=500, unit='cm', device='png')
...
df_individual <- load_forecasts(models = c("COVIDhub-ensemble"),
                                targets = "1 wk ahead cum death",
                                exclude_locations = c("11", "60", "66", "69", "72", "74", "78"), 
                                start_date="2020-08-29", end_date='2021-02-06') %>% 
  select(-c(forecast_date, type)) %>%
  select(target_end_date, everything())
...
df_individual <- load_forecasts(models = c("COVIDhub-ensemble"),
                                ...
                                exclude_locations = c("11", "60", "66", "69", "72", "74", "78"), 
                                start_date="2020-11-21", end_date='2021-03-27') %>% 
  select(-c(forecast_date, type)) %>%
  select(target_end_date, everything())
...
write.csv(individual_scores, "scores/evaluation_study/1wk_inc_case/covidhub-ensemble_scores_1wk_inc_case.csv", row.names=FALSE)

write.csv(individual_scores, "scores/evaluation_study/1wk_cum_death/covidhub-ensemble_scores_1wk_cum_death.csv", row.names=FALSE)
...
df_ensembles <- load_ensembles("data/ensemble_forecasts/evaluation_study/df_ensembles_1wk_noUS_top5_ws1234.csv", 
                               add_baseline = TRUE)
ensemble_scores <- score_forecasts(df_ensembles)
write.csv(ensemble_scores, "scores/evaluation_study/ensemble_scores_1wk_noUS_top5.csv", row.names=FALSE)

df3 <- load_scores("scores/evaluation_study/ensemble_scores_1wk_noUS_top3_ws4.csv", remove_revisions=TRUE, long_format=TRUE)
df5 <- load_scores("scores/evaluation_study/ensemble_scores_1wk_noUS_top5.csv", remove_revisions=TRUE, long_format=TRUE)


plot_wis(df5, locations='states', x=window_size, facet=model, 
         ncol=3, hjust=0.5, dir='h', title=NULL)

ggplot(subset(df5, location !='US' & window_size==4 & score %in% c("wgt_pen_l", "wgt_iw", "wgt_pen_u")), 
       aes(x=reorder(model, value), y=value,
           fill=factor(score, levels=c("wgt_pen_l", "wgt_iw", "wgt_pen_u")))) +
  geom_bar(position="stack", stat="summary", fun=mean, width=0.7) +
  theme_gray(base_size=10) +
...
a <- subset(df3, location !='US' & window_size==4 & score=='wis') %>%
  group_by(target_end_date, model) %>%
  summarize(wis=mean(value))

ggplot(a, 
       aes(x=target_end_date, y=wis,
           colour=model)) +
  geom_line() +
  theme_gray(base_size=10) +
  theme(axis.text.x=element_text(vjust=0.5, angle=90, hjust=1), 
...
df5$model <- paste0(df5$model, '-5F')

df_all <- bind_rows(df1, df2, df3)
df_all <- bind_rows(df5, df3)
...
ggsave('plots/evaluation_study/mean_wis_1wk_3F-5F.png', width=20, height=15, dpi=500, unit='cm', device='png')
...
df_individual <- subset(df, target_end_date >= '2020-08-29')
individual_scores <- score_forecasts(df_individual)
...
individual_scores <- load_scores("scores/evaluation_study/individual_scores_1wk.csv", remove_revisions=TRUE, long_format=TRUE)

ggplot(subset(individual_scores, location != 'US' & score %in% c("wgt_pen_l", "wgt_iw", "wgt_pen_u")), 
       aes(x=reorder(model, value), y=value,
           fill=factor(score, levels=c("wgt_pen_l", "wgt_iw", "wgt_pen_u")))) +
  geom_bar(position="stack", stat="summary", fun=mean, width=0.7) +
...
df <- load_scores("scores/evaluation_study/ensemble_scores_1wk_noUS_all_ws4_is_unsorted_new2.csv", 
                  remove_revisions=FALSE, long_format=TRUE, in_sample=TRUE)
...
ggplot(subset(df, location != 'US' & window_size==4 & score %in% c("wgt_pen_l", "wgt_iw", "wgt_pen_u")), 
  ...
  labs(x = NULL,
       y = "Mean WIS") +
  ggtitle("In-sample performance (unsorted)")

ggsave('plots/evaluation_study/mean_wis_1wk_ensembles_inSample_unsorted.png', width=20, height=15, dpi=500, unit='cm', device='png')
...
View(subset(scores, model %in% c('V3_3F', 'GQRA2', 'EWA_3F', 'EWA', 'COVIDhub-ensemble', 'COVIDhub-baseline')))
...
e <- subset(ensemble_scores, model %in% c("V4", "QNA3"))
e <- e %>%
  select(c(target_end_date, location, model, wis))

e <- pivot_wider(e, id_cols=c(target_end_date, location), names_from=model, values_from=wis)

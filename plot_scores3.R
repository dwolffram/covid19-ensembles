...
df1 <- load_scores("scores/evaluation_study/individual_scores_1wk.csv", remove_revisions=TRUE, long_format=TRUE)
df2 <- load_scores("scores/evaluation_study/COVIDhub-ensemble_scores_1wk.csv", remove_revisions=TRUE, long_format=TRUE)

df3 <- load_scores("scores/evaluation_study/ensemble_scores_1wk_noUS_top3_ws4.csv", remove_revisions=TRUE, long_format=TRUE)
df4 <- load_scores("scores/evaluation_study/ensemble_scores_1wk_noUS_all_ws4_oos.csv", remove_revisions=TRUE, long_format=TRUE)
...
df3 <- df3 %>%
  filter(window_size == 4 & model != 'Baseline') %>%
  select(-window_size)
...
df3$model <- paste0(df3$model, '_3F')
df5$model <- paste0(df5$model, '-5F')

df1$type <- "Individual Model"
df1[df1$model == "COVIDhub-baseline", ]$type <- "Reference"
df2$type <- "Reference"
df3$type <- "Subset-Ensemble"
df4$type <- "Ensemble"


df_all <- bind_rows(df1, df2, df3, df4)
df_all <- subset(df_all, location != 'US')
...
df_wis <- df_all %>%
  filter(score == "wis") %>%
  group_by(target_end_date, type, model) %>%
  summarize(meanWIS = mean(value)) 


ggplot(subset(df_wis, type != 'Individual Model'), aes(x=reorder(model, meanWIS), fill=type, y=meanWIS)) + 
...
df_rank <- subset(df_all, type!= 'Individual Model') %>%
  filter(score == "wis") %>%
  group_by(target_end_date, type, model) %>%
  summarize(meanWIS = mean(value)) %>%

...
wis <- function(df){
  ...
  if (str_detect(unique(df$target), 'inc case')){
    coverage_levels <- c(0.5, 0.8, 0.95)
  } else {
    coverage_levels <- c(1:9/10, 0.95, 0.98) 
  }
  ...
  df_wide$wgt_iw <- rowSums(df_wide[, grepl("wgt_iw", colnames(df_wide))])/(length(coverage_levels) + 1/2)
  df_wide$wgt_pen_u <- rowSums(df_wide[, grepl("wgt_pen_u", colnames(df_wide))])/(length(coverage_levels) + 1/2)
  df_wide$wgt_pen_l <- rowSums(df_wide[, grepl("wgt_pen_l", colnames(df_wide))])/(length(coverage_levels) + 1/2)
